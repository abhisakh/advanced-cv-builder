"""
Advanced CV Builder with AI Enhancement, Built-in Text Formatter,
...
with st.expander("🔗 Profiles & Links", expanded=True):
    col_github, col_linkedin, col_portfolio = st.columns(3)
...
# -------- CUSTOM SECTIONS DATA INPUT (DYNAMIC STRUCTURE) --------
...
# Calculate variable safe_filename globally for export functions
...
with col_ai:
    if st.button("✨ Get AI Enhancement Suggestions", use_container_width=True):
        with st.spinner("Analyzing your CV..."):
            ai_suggestions = get_cv_enhancement_suggestions(cv_data)
    ...
    if st.session_state.show_ai_suggestions:
        with st.expander("🤖 AI Suggestions", expanded=True):
            st.info(get_cv_enhancement_suggestions(cv_data))
...
def render_single_section(sec_name, sections_data):
    sec_html = ""
...
def generate_cv_html(cv_data, template_config, photo_settings, sidebar_width_pct=32, sidebar_position="Right"):
    ...
    for sec in st.session_state.section_order:
        if st.session_state.section_visibility.get(sec, True):
            rendered = render_single_section(sec, cv_data.get("sections_data", {}))
    ...
    meta_extra = f'<br/>{residency} | {relocation}' if (residency or relocation) else ''
...
with col_export:
    ...
    with st.popover("📝 Save Version", use_container_width=True):
        version_input = st.text_input("Version Tag", value=f"{safe_filename}_{st.session_state.selected_template}")
        if st.button("Confirm Save", use_container_width=True):
            save_version(cv_data, version_input)
            st.success(f"✅ Saved version: {version_input}")
